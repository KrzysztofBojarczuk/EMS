...
{
    public record GetUserVehiclesQuery(string appUserId, int pageNumber, int pageSize, List<VehicleType> vehicleType, string searchTerm) : IRequest<PaginatedList<VehicleEntity>>;
    ...
    public class GetUserVehiclesQueryHandler(IVehicleRepository vehicleRepository) : IRequestHandler<GetUserVehiclesQuery, PaginatedList<VehicleEntity>>
    {
        ...
        {
           return await vehicleRepository.GetUserVehiclesAsync(request.appUserId, request.pageNumber, request.pageSize, request.vehicleType, request.searchTerm);
        }
    }
}