...
{
    public record GetUserReservationsQuery(string appUserId, int pageNumber, int pageSize, string searchTerm, string sortOrder) : IRequest<PaginatedList<ReservationEntity>>;
    ...
    public class GetUserReservationsQueryHandler(IReservationRepository reservationRepository) : IRequestHandler<GetUserReservationsQuery, PaginatedList<ReservationEntity>>
    {
        ...
        {
            return await reservationRepository.GetUserReservationsAsync(request.appUserId, request.pageNumber, request.pageSize, request.searchTerm, request.sortOrder);
        }
    }
}