...
{
    public record GetUserEmployeesQuery(string appUserId, int pageNumber, int pageSize, string searchTerm, string sortOrderSalary) : IRequest<PaginatedList<EmployeeEntity>>;
    ...
    public class GetUserEmployeesQueryHandler(IEmployeeRepository employeeRepository) : IRequestHandler<GetUserEmployeesQuery, PaginatedList<EmployeeEntity>>
    {
        ...
        {
            return await employeeRepository.GetUserEmployeesAsync(request.appUserId, request.pageNumber, request.pageSize, request.searchTerm, request.sortOrderSalary);
        }
    }
}